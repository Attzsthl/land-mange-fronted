<template>
     <d2-container>
        ...
        <el-table :data="templateData" v-loading="loading" class="table" ref="templateTable" >
                ...
                <el-table-column prop="shareholderName" label="股东姓名" width="150">
                </el-table-column>
                <el-table-column prop="idCard" label="身份证号码" min-width="100">
                </el-table-column>
                <el-table-column prop="familyNum" label="家庭成员个数" width="200">
                </el-table-column>
                <el-table-column  label="当前股份类型" min-width="100">
                  <template slot-scope="scope">
                        <span> {{ getSocialType(scope.row.presentStockType) }}</span>
                  </template>
                </el-table-column>
                <el-table-column  label="变更类型" width="100">
                  <template slot-scope="scope">
                        <span> {{ getSocialType(scope.row.newStockType) }}</span>
                  </template>
                </el-table-column>
         ...
         <el-row>
          <el-col :span="24" style="text-align: center; margin-top: 15px;">
               <el-button
                        type="primary"
                        size="custom"
                        @click="nextStep">下一步：计算地面附着物赔偿资金</el-button>
          </el-col>
        </el-row>
     </d2-container>
</template>
...
<script>
import { getStockPage } from '@api/exit.mange.js'
